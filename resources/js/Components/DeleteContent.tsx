...
export const DeleteContent = ({ editor }: { editor: Editor }) => {

    return <button
        className="py-1 px-1 rounded-md cursor-pointer hover:bg-slate-200"
        onClick={() => editor.chain().focus().deleteRange({
            from: editor.state.selection.from,
            to: editor.state.selection.to
        }).run()}
    >
        <svg className="w-4 stroke-red-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
