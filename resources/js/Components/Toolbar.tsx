            ...
            <div className="p-2 flex items-end">
                <div id="history" className="mr-5">
                    <button
                        ...
                        className={` w-6 h-6 p-1 rounded-full shadow-md cursor-pointer hover:bg-indigo-500 disabled:bg-slate-200  disabled:hover:bg-white `}>
                ...
                <div className="p-2 bg-white/70 rounded-sm shadow-md">
                    ...
                    <div className="flex py-2  max-w-max max-h-max text-slate-800 px-2   space-x-1 ">
                        <button
                            ...
                            className={`${editor?.isActive('bold') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer  w-6 h-6 flex justify-center items-center rounded-sm text-sm p-1 font-bold`}>
                            b
                        </button>
                        <button
                            onClick={() => editor?.chain().focus().toggleItalic().run()}
                            className={`${editor?.isActive('italic') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer  w-6 h-6 flex justify-center items-center rounded-sm text-sm p-1 italic`}>
                            i
                        </button>
                        <button
                            onClick={() => editor?.chain().focus().toggleUnderline().run()}
                            className={`${editor?.isActive('underline') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer  w-6 h-6 flex justify-center items-center rounded-sm text-sm p-1 `}>
                            <u>u</u>
                        </button>
                        <button
                            onClick={() => editor?.chain().focus().toggleStrike().run()}
                            className={`${editor?.isActive('strike') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer  w-6 h-6 flex justify-center items-center rounded-sm text-sm p-1 `}>
                            <s>s</s>
                        </button>
                    </div>
                </div>
                <div className="p-2 bg-white/70 rounded-sm shadow-md">
                    <div className="font-bold">Headings</div>
                    <div className="flex space-x-1 text-slate-800 px-2">
                        <button
                            onClick={() => editor?.chain().toggleHeading({ level: 2 }).run()}


                            className="cursor-pointer hover:bg-slate-100 p-1 rounded-sm font-semibold ">1.1
                            Section</button>
                        <button
                            onClick={() => editor?.chain().toggleHeading({ level: 3 }).run()}
                            className="cursor-pointer hover:bg-slate-100 p-1 rounded-sm font-semibold ">1.1.1
                            Subsection</button>
                    </div>
                </div>
                <div className="p-2 bg-white/70 rounded-sm shadow-md">
                    ...
                    <div className="flex  space-x-1 text-slate-800 px-2">
                        <button
                            ...
                            className={`${editor?.isActive('orderedList') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer flex justify-center items-center space-x-1 hover:bg-slate-100 p-1 rounded-sm `}>
                        ...
                        <button
                            ...
                            className={`${editor?.isActive('bulletList') ? 'bg-indigo-700 text-white hover:bg-indigo-600' : 'hover:bg-slate-200'} cursor-pointer flex justify-center items-center space-x-1 hover:bg-slate-100 p-1 rounded-sm `}>
                            <svg className="w-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                                <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
