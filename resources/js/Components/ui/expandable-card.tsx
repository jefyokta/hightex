...
const TrashButton: React.FC<{ id: number, active: boolean }> = ({ id, active }) => {
    ...
    return <motion.button
        onClick={() => { Delete(id) }}
    >
        <motion.div transition={{ duration: 0.4 }} className={active ? "opacity-100" : "opacity-0"}>
            <motion.div className="border-gray-200 w-10 h-10  flex items-center justify-center py-1 px-2.5 border rounded-full">
        ...
        <>
            ...
            <AnimatePresence>
                {active && (
                    <div
                        className={cn(
                            "fixed inset-0 grid place-items-center z-[100]  before:pointer-events-none",
                        )}
                    >
                        <motion.div
                            ...
                            <div className="relative h-full before:fixed before:inset-x-0 before:bottom-0 before:h-[70px] before:z-50 before:bg-gradient-to-t dark:before:from-zinc-950 before:from-zinc-50">
                                <div className="flex justify-between items-start p-8 h-auto">
                                    ...
                                    <div className="flex space-x-5">
                                        <TrashButton id={1} active={active}
                                        />
                                        <motion.button
            ...
            <motion.div
                ...
                className={cn(
                    "p-3 flex flex-col w-96 justify-between z-9 items-center bg-zinc-50 shadow-sm dark:shadow-none dark:bg-zinc-950 rounded-2xl cursor-pointer border border-gray-200/70 dark:border-zinc-900",
                    className,
                )}
            >
                <div className="flex gap-4 flex-col boder ">
                    <motion.div layoutId={`image-${title}-${id}`}>
                        <img
                            src={src}
                            alt={title}
